...
static void UART_Emul_Init ()
{
  uint32_t bit_time = 0;
  bit_time = ((uint32_t) ((168000000/115200) - 1));	//Hz
...
/**
  ...
  * @retval None
  */
static void UART_Emul_TransmitFormatFrame(uint8_t Data, uint32_t *pBuffer_Tx)
{
uint32_t counter = 0;
uint32_t bitmask = 0;
uint32_t length = 0;
...
/////////////////////////////////////////////////////////////////////////////////////////ne hvat bufer
static void UART_Emul_TransmitFrame()
{
  uint32_t tmp_sr = 0;
  uint32_t tmp_ds = 0;
  uint32_t tmp_size = 0;
...
HAL_StatusTypeDef HAL_UART_Emul_Transmit_DMA(uint8_t *pData, uint16_t Size)
{
  ...
  if ((tmp == HAL_UART_EMUL_STATE_READY) || (tmp == HAL_UART_EMUL_STATE_BUSY_RX))
  {
    ...
    if (huart->TxXferCount == FIRST_BYTE)
    {
      ...
      UART_Emul_TransmitFormatFrame(*(pData), (uint32_t*)pFirstBuffer_Tx);
