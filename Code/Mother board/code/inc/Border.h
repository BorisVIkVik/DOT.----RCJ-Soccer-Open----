...
class Border
{
    public:
        ...
        void dempher(pair<int16_t, int16_t> pos, VectorToMove& vtm, double& acc, int32_t lineSensorsValue);
				void Goaldempher(pair<int16_t, int16_t> pos, VectorToMove& vtm, double& acc, int32_t lineSensorsValue);
    private:
				double _speed;
        char _coord;
        char _side;
...
void Border::Goaldempher(pair<int16_t, int16_t> pos, VectorToMove& vtm, double& acc, int32_t lineSensorsValue)
{
    if(_coord == 'x')
    {
        double error = double(pos.X - _start)/double(_end - _start);				
